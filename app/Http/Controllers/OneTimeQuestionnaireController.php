...
class OneTimeQuestionnaireController extends Controller
{
    public function __invoke(Request $request)
    {
        ...
        try {
            ...
            foreach (
                $request->mental_health_conditions
                as $mentalHealthConditionKey => $mentalHealthConditionValue
            ) {
                $userHealthProfile->$mentalHealthConditionKey = $mentalHealthConditionValue;
            }

            foreach (
                $request->health_problems_or_disabilities
                as $healthPromlemOrDisabilityKey =>
                    $healthPromlemOrDisabilityValue
            ) {
            ...
            $user->socioDemographicProfile()->create([
                ...
                "difficulty_managing_household_income" =>
                    $request->difficulty_managing_household_income,
                "highest_level_of_education" =>
                    $request->highest_level_of_education,
                "employment_status" => $request->employment_status,
                "self_specified_ethnic_group" =>
                    $request->self_specified_ethnic_group,
                "ethnic_group" => $request->ethnic_group,
            ]);

            $user->metaProfile()->create([
                "for_physical_health" => $request->for_physical_health,
                "for_mental_health_wellbeing" =>
                    $request->for_mental_health_wellbeing,
                "for_social_isolation" => $request->for_social_isolation,
                "for_lifestyle_change" => $request->for_lifestyle_change,
                "for_self_care_self_management" =>
                    $request->for_self_care_self_management,
                "for_social_welfare_advice" =>
                    $request->for_social_welfare_advice,
                "for_financial_advice" => $request->for_financial_advice,
                "for_work" => $request->for_work,
                "for_training_and_learning" =>
                    $request->for_training_and_learning,
                "for_prefer_not_to_say" => $request->for_prefer_not_to_say,
                "for_self_specified_reason" =>
                    $request->for_self_specified_reason,
            ]);
